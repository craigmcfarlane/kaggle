...
'''
features to add
embarked, title - edit, hasCabin, parch, parch + sib = notAlone
to get title - splice after comma to next whitespace?, remove period if present, standardize titles
hasCabin
'''
X_init = train[features]
...
binner4 = KBinsDiscretizer(n_bins=4)
X_fare_bin = binner4.fit_transform(X_fare)
X.drop(1, axis=1)
X = X + X_fare_bin


'''
be sure of what this following code is doing...its runs but not sure
'''
...
xg = SklearnWrapper(clf=XGBClassifier, seed=SEED, params=xgb_params)
